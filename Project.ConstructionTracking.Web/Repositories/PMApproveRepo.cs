        ...
        public ApproveFormcheckModel GetApproveFormcheck(ApproveFormcheckModel model)
        {
            var result = (from t1 in _context.tr_UnitForm
                          ...
                          select new ApproveFormcheckModel
                          {
                              ...
                              PM_getListQCData = (from t1 in _context.tr_Form_QCCheckList
                                                  join t2 in
            ...
            using (var scope = new TransactionScope(TransactionScopeOption.Required, transactionOptions))
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        if (!string.IsNullOrEmpty(model.Remark) && unitFormAction.Remark != model.Remark)
                        {
                    ...
                    if (model.PassConditionsIUD != null && model.PassConditionsIUD.Count > 0)
                    {
                        foreach (var passConditionModel in model.PassConditionsIUD)
                        {
                            ...
                            if (passCondition != null && passCondition.StatusID != 8)
                            {
                                passCondition.StatusID = passConditionModel.PassConditionsvalue;
                                if (!string.IsNullOrEmpty(passConditionModel.Remark) && passCondition.PM_Remark != passConditionModel.Remark)
                                {
        ...
        public PMRespond GetPMRespondSendEmailData(Guid unitFormId)
        {
              var result =  (from t1 in _context.tr_UnitForm
                             ...
                             select new PMRespond
                             {
                                 ...
                                 //PEEmail = "sittikron.p@assetwise.co.th",
        ...
        public List<QCnotifyPMSubmit> GetListQCnotifyPMSubmitlData(int FormID, Guid UnitID, Guid ProjectID)
        {
            ...
            if (CheckQCform.Count > 0)
            {
                var result = (from t1 in _context.tr_QC_UnitCheckList
                              ...
                              from t11 in t11Group.DefaultIfEmpty()
                              join t12 in _context.tr_RoleActionStatus on t10.StatusID equals t12.ID into t12Group
                              from t12 in t12Group.DefaultIfEmpty()
                              where t1.UnitID == UnitID
                                    && t10.RoleID == SystemConstant.UserRole.PM
                                    && t3.FormID == FormID
                                    && t3.FormID != null
                              select new QCnotifyPMSubmit
                              {
                                  ...
                                  ProjectName = t7.ProjectName ?? string.Empty,
                                  UnitCode = t8.UnitCode ?? string.Empty,
                                  PMUserName = (t11.FirstName ?? string.Empty) + " " + (t11.LastName ?? string.Empty),
                                  StatusID = t10.StatusID,
                                  StatusName = t12.Name ?? string.Empty,
                ...
                if (result == null)
                {
                    result = (from t1 in _context.tr_UnitForm
                              ...
                              join t5 in _context.tm_Unit on t1.UnitID equals t5.UnitID into t5Group
                              from t5 in t5Group.DefaultIfEmpty()
                              join t6 in _context.tr_UnitFormAction.Where(t6 => t6.RoleID == SystemConstant.UserRole.PM) on t1.ID equals t6.UnitFormID into t6Group
                              from t6 in t6Group.DefaultIfEmpty()
                              join t7 in _context.tm_User on t6.UpdateBy equals t7.ID into t7Group
                              from t7 in t7Group.DefaultIfEmpty()
                              join t8 in _context.tm_Form on t1.FormID equals t8.ID into t8Group
                              ...
                              from t11 in t11Group.DefaultIfEmpty()
                              join t12 in _context.tr_RoleActionStatus on t6.StatusID equals t12.ID into t12Group
                              from t12 in t12Group.DefaultIfEmpty()
                              where t1.UnitID == UnitID
                              ...
                              select new QCnotifyPMSubmit
                              {
                                  ...
                                  PMUserName = (t7.FirstName ?? string.Empty) + " " + (t7.LastName ?? string.Empty),
                                  StatusID = t6.StatusID,
                                  StatusName = t12.Name ?? string.Empty,
            ...
            {
                return new List<QCnotifyPMSubmit>();
            }
        }
